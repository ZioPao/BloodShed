...
class ABL_AnimatedDecalManager : GenericEntity
{
	...
	void ABL_AnimatedDecalManager(IEntitySource src, IEntity parent)
	{
		...
		animationMaterials.Insert(EDecalType.GENERIC_SPLATTER, {"{8F3E9CE433017B40}TestWallSplatterAlpha/001.emat","{D1D6B5A78C8CF54F}TestWallSplatterAlpha/002.emat",
"{25DEF23F81519D3B}TestWallSplatterAlpha/003.emat",
"{6C06E720F397E951}TestWallSplatterAlpha/004.emat",
"{980EA0B8FE4A8125}TestWallSplatterAlpha/005.emat","{C6E689FB41C70F2A}TestWallSplatterAlpha/006.emat",
		"{6FBB0C084B4B43D2}TestWallSplatterAlpha/007.emat",
		"{5556A3C5A44BE7FE}TestWallSplatterAlpha/008.emat"});
	...
	void StartNewAnimation(IEntity character, vector hitPosition, vector hitDirection, EDecalType type, bool terrainOnly, float sizeModifier, int nodeId)
	{
		...
		switch(type)
		{
			...
			case EDecalType.GENERIC_SPLATTER:
			{
				...
				alphaTestValue = 0.8;
				indexAlpha = Math.RandomIntInclusive(0, animationMaterials.Get(EDecalType.GENERIC_SPLATTER).Count() - 1);			//todo make this dynamic
	...
	void SpawnAnimatedFrames()
	{
		...
		foreach(int index, DecalInformation dInfo  : decalsSpawned)
		{
			...
			array<ResourceName> tempFrames = animationMaterials.Get(dBaseInfo.type);
			settings = MCF_SettingsManager.GetInstance().GetModSettings(ABL_MOD_ID);			//refresh!
			...
			if (dMaterialInfo.alphaMul < maxAlphaTest)
			{
				...
				if (traceParam.TraceEnt) // spawn splatter below character
				{
					...
					tmp.SetParam("AlphaTest", alphaTest);		//todo this is wrong, there is not AlphaBlend. Do you mean AlphaTest?
