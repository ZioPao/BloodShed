...
class BS_AnimateBloodManager : GenericEntity
{
	...
	void BS_AnimateBloodManager(IEntitySource src, IEntity parent)
	{
		...
		materialsMap.Insert(EDecalType.SINGLE_FRAME_GENERIC_SPLATTER, {"{B3196D0D8CB491D5}materials/ground_splatter/001.emat", "{C64AA3033AED2B24}materials/ground_splatter/002.emat", "{3242E49B37304350}materials/ground_splatter/003.emat"});
		
		
		//lets load everything right here?
		bloodsplattersLoaded = new array<Material>();
		wallsplattersLoaded = new array<Material>();

		
		
		Print("Loading Bloodpools");
		foreach (ResourceName rsBloodpool : materialsMap.Get(EDecalType.BLOODPOOL))
			bloodsplattersLoaded.Insert(Material.GetOrLoadMaterial(rsBloodpool, 0));
		
		Print("Loading wall splatters");
		foreach (ResourceName rsWallsplatter : materialsMap.Get(EDecalType.WALLSPLATTER))
			wallsplattersLoaded.Insert(Material.GetOrLoadMaterial(rsWallsplatter, 0));
	...
	void StartNewAnimation(IEntity character, vector hitPosition, vector hitDirection, EDecalType type, bool terrainOnly, float sizeModifier, int nodeId)
	{
		...
		switch(type)
		{
			...
			case EDecalType.WALLSPLATTER:
			{
				...
				SCR_ParticleEmitter particleEmitter = SCR_ParticleAPI.PlayOnPositionPTC("{2A1C546089927ECC}Particles/Enviroment/blood_wall_dripping.ptc", intersectionPosition);
